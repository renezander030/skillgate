...
set -eu
# Pin the gate version (override with SKILLGATE_VERSION=...). Keeps the box's
# skillgate consistent with the pinned default in pre-receive.
...
if [ -f /tmp/done.yaml ]; then
  ...
  npx -y "@reneza/skillgate@$SKILLGATE_VERSION" init >/dev/null 2>&1 || true
